...
document
  ...
  .addEventListener('submit', async function (r) {
    r.preventDefault();
    const l = document.getElementById('name').value.trim().toLowerCase(),
      i = document.getElementById('school').value.trim().toLowerCase(),
      m = document.getElementById('email').value.trim(),
      c = await p(u(n, 'players'));
    let e = null;
    c.forEach(a => {
      const t = a.data();
      t.name?.toLowerCase() === l &&
        t.school?.toLowerCase() === i &&
        t.placeholder === !0 &&
        (e = { id: a.id, ...t });
    });
...
//# sourceMappingURL=claim-profile-Bz-_hgNK.js.map
