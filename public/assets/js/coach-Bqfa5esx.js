...
import {
  ...
  where as w,
  setDoc as j,
  serverTimestamp as q,
  Timestamp as z,
} from 'https://www.gstatic.com/firebasejs/9.22.2/firebase-firestore.js';
import { getStorage as R } from 'https://www.gstatic.com/firebasejs/9.22.2/firebase-storage.js';
import { l as L } from './teamComparison-DkPk5Wsi.js';
...
const v = localStorage.getItem('coachId') || 'demoCoach',
  T = localStorage.getItem('team') || 'Demo Team';
L('visit', v, 'coach');
(async function () {
  const e = l(c, 'coaches', v),
    a = await u(e);
  if (!a.exists()) {
...
async function B() {
  const t = await d(y(p(c, 'games'), w('teamName', '==', T))),
...
window.viewGameHighlights = async function (t) {
  ...
  ((e.innerHTML = `<h3>🎬 Highlights for Game: ${t}</h3>`),
    (await d(p(c, `games/${t}/clips`))).forEach(o => {
      const s = o.data(),
        n = document.createElement('div');
...
window.viewPlayerBreakdowns = async function (t) {
  ...
  const a = await d(p(c, `playerStats/${t}/plays`)),
    o = {};
  a.forEach(s => {
    const n = s.data();
    (o[n.playerId] || (o[n.playerId] = []), o[n.playerId].push(n));
  });
  for (const [s, n] of Object.entries(o)) {
    const m = document.createElement('div');
    m.innerHTML = `<strong>${s}</strong><ul>${n.map(g => `<li>${g.type} at ${g.timestamp}s</li>`).join('')}</ul>`;
    const Q = l(c, `scoutingReports/${t}_${s}`),
      $ = await u(Q);
    if ($.exists()) {
      const g = $.data();
      m.innerHTML += `<p><strong>📋 Scouting Report:</strong><br>${g.report}<br><em>Grade: ${g.grade}</em></p>`;
    }
    e.appendChild(m);
  }
  document.body.appendChild(e);
};
const k = document.getElementById('uploadForm');
k &&
  k.addEventListener('submit', async t => {
    t.preventDefault();
    const e = document.getElementById('gameId').value.trim(),
      a = document.getElementById('teamName').value.trim(),
      o = document.getElementById('opponent').value.trim(),
      s = document.getElementById('gameDate').value,
      n = document.getElementById('videoUrl').value.trim(),
      m = l(c, 'games', e);
    (await j(m, {
      teamName: a,
      opponent: o,
...
const Z = {
  ...
  _ = i(Z),
  F = r(_),
  J = localStorage.getItem('team') || 'Demo Team',
  V = y(p(F, 'games'), w('teamName', '==', J)),
  X = await d(V),
  E = [],
  x = [],
  M = [],
...
X.forEach(t => {
  ...
  (E.push(e.gameDate || 'Game'),
    x.push(e.stats?.points || 0),
    M.push(e.stats?.assists || 0),
    A.push(e.stats?.rebounds || 0),
    H.push(e.stats?.steals || 0),
    G.push(e.stats?.blocks || 0));
});
const U = document.getElementById('teamChart').getContext('2d');
new I(U, {
  type: 'line',
  data: {
    labels: E,
    datasets: [
      { label: 'Points', data: x, borderColor: 'blue', fill: !1 },
      { label: 'Assists', data: M, borderColor: 'green', fill: !1 },
...
const Y = {
  ...
  ae = p(ee, 'analyticsLog'),
  f = {},
  h = new Date();
h.setDate(h.getDate() - 14);
const oe = z.fromDate(h),
  ne = y(ae, w('timestamp', '>', oe)),
  se = await d(ne);
se.forEach(t => {
  const a = t.data().timestamp.toDate().toLocaleDateString();
  f[a] = (f[a] || 0) + 1;
});
const ce = Object.keys(f),
  ie = Object.values(f);
new I(te, {
  type: 'bar',
  data: {
...
const re = {
  ...
  le = i(re),
  de = r(le),
  pe = localStorage.getItem('coachId') || 'demoCoach',
  me = y(p(de, 'games'), w('uploadedBy', '==', pe)),
  ge = await d(me),
  P = [],
  N = [];
ge.forEach(t => {
  ...
  const a = new Date(e.gameDate);
  (P.push(a.toISOString().split('T')[0]),
    N.push(e.result.toLowerCase() === 'win' ? 1 : 0));
});
...
const be = {
  ...
  we =
    ...
    'demoPlayer',
  he = l(ye, 'advancedMetrics', we),
  C = document.getElementById('metricsViewer');
u(he).then(t => {
  if (t.exists()) {
    const e = t.data();
    C.innerHTML = `
    ...
    `;
  } else C.textContent = 'No advanced metrics available yet.';
});
const Ie = {
    apiKey: 'AIzaSyD4XJLc3_CLGvOhMysQTx2fabgZQt3y5g0',
    authDomain: 'ball-network-web.firebaseapp.com',
  ...
  ve = i(Ie),
  O = r(ve),
  $e = localStorage.getItem('team') || 'Demo Team',
  ke = l(O, 'teams', $e),
  S = await u(ke),
  Ce = S.exists() ? S.data() : {},
  Se = document.getElementById('teamTags');
Se.innerHTML = (Ce.tags || []).map(t => `<li>${t}</li>`).join('');
const Le = localStorage.getItem('playerId') || 'demoPlayer',
  b = document.getElementById('chatBox');
window.sendCoachMessage = async function () {
  const t = document.getElementById('chatInput'),
    e = t.value.trim();
  if (!e) return;
  const a = l(O, 'players', Le),
    o = await u(a),
    s = o.exists() ? o.data() : {},
    n = await De(e, s);
  ((b.innerHTML += `<div><strong>You:</strong> ${e}</div>`),
    (b.innerHTML += `<div><strong>Coach:</strong> ${n}</div>`),
    (b.scrollTop = b.scrollHeight),
    (t.value = ''));
};
async function De(t, e) {
  const a = e.avgGrade || 0,
    o =
      a >= 70
        ? 'Keep up the great work and focus on consistency!'
        : 'Let’s work on defense and shot selection this week.';
  return `Your average grade is ${a}. ${o}`;
}
