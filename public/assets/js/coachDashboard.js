...
(async function checkApproval() {
  ...
  if (!snap.exists()) {
    document.body.innerHTML = '<h2 style="text-align:center;margin-top:100px;">❌ Coach profile not found.</h2>';
    return;
  }
  const data = snap.data();
  if (data.approved === false) {
    document.body.innerHTML = '<h2 style="text-align:center;margin-top:100px;">⏳ Your account is pending admin approval.</h2>';
    return;
  }
  loadTeamGames();
})();
...
async function loadTeamGames() {
  const gameSnap = await getDocs(query(collection(db, "games"), where("teamName", "==", coachTeamName)));
...
window.viewPlayerBreakdowns = async function(gameId) {
  const breakdown = document.getElementById("gameBreakdownView") || document.createElement("div");
  ...
  snap.forEach(doc => {
    const d = doc.data();
    if (!byPlayer[d.playerId]) byPlayer[d.playerId] = [];
