import './modulepreload-polyfill-B5Qt9EMX-ulV_1b7r.js';
import { initializeApp as f } from 'https://www.gstatic.com/firebasejs/9.22.2/firebase-app.js';
import {
  getFirestore as w,
  getDoc as l,
  doc as d,
  getDocs as h,
  query as D,
  collection as I,
} from 'https://www.gstatic.com/firebasejs/9.22.2/firebase-firestore.js';
/* empty css                       */ const b = {
    apiKey: 'AIzaSyD4XJLc3_CLGvOhMysQTx2fabgZQt3y5g0',
    authDomain: 'ball-network-web.firebaseapp.com',
    projectId: 'ball-network-web',
    storageBucket: 'ball-network-web.appspot.com',
    messagingSenderId: '740915998465',
    appId: '1:740915998465:web:59ac026f3f4c2ec5da3500',
  },
  k = f(b),
  n = w(k),
  S = localStorage.getItem('fanId') || 'demoFan',
  s = await l(d(n, 'fans', S)),
  c = s.exists() ? s.data() : {},
  m = c.playersFollowing || [],
  A = c.teamsFollowing || [],
  E = document.getElementById('fanHighlights'),
  B = document.getElementById('fanReports'),
  p = new Date().toDateString();
let g = !1,
  u = !1;
const o = [];
for (let e of m) {
  const t = d(n, 'players', e),
    i = await l(t);
  if (!i.exists()) continue;
  const a = i.data();
  (a.highlights || []).forEach(y => {
    (o.push({
      name: a.name || e,
      link: y,
      date: a.updatedAt || new Date().toISOString(),
    }),
      a.updatedAt && new Date(a.updatedAt).toDateString() === p && (g = !0));
  });
}
o.sort((e, t) => new Date(t.date) - new Date(e.date));
o.forEach(e => {
  E.innerHTML += `<a class='highlight-link' href='${e.link}' target='_blank'>🎬 ${e.name}</a>`;
});
g && (document.getElementById('alertHighlights').style.display = 'block');
const v = await h(D(I(n, 'scoutingReports'))),
  r = [];
v.forEach(e => {
  const t = e.data();
  (m.includes(t.playerId) || A.includes(t.teamName)) &&
    (r.push({
      summary: t.summary,
      player: t.playerName || t.teamName,
      date: t.updatedAt || new Date().toISOString(),
    }),
    t.updatedAt && new Date(t.updatedAt).toDateString() === p && (u = !0));
});
r.sort((e, t) => new Date(t.date) - new Date(e.date));
r.forEach(e => {
  B.innerHTML += `<div><strong>${e.player}</strong>: ${e.summary}</div><br>`;
});
u && (document.getElementById('alertReports').style.display = 'block');
//# sourceMappingURL=fan-Cavv95E5.js.map
