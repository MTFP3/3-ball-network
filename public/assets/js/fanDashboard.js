...
async function loadFanData() {
  const q = query(collection(db, "players"), orderBy("rating", "desc"), limit(10));
...
function renderTopPlayers(players) {
  topPlayersDiv.innerHTML = players.map(p => `
...
function renderHighlights(players) {
  highlightGallery.innerHTML = players.map(p => {
...
function renderLeaderboard(players) {
  leaderboard.innerHTML = players.map(p => `
