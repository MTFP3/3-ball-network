...
async function loadPlayerData() {
  ...
  if (data.approved === false) {
    document.body.innerHTML = '<h2 style="text-align:center;margin-top:100px;">⏳ Your account is pending approval by an admin.</h2>';
    return;
  }
  ...
  if (statsList) {
    statsList.innerHTML = Object.entries(stats).map(([k, v]) => `<li>${k}: ${v}</li>`).join('');
  }
...
async function loadDailyReport() {
  ...
  try {
    const url = await getDownloadURL(storageRef(storage, `reports/${playerId}/daily.pdf`));
    link.href = url;
