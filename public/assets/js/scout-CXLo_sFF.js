...
import { M as w } from './teamComparison-DkPk5Wsi-DTB9EFa3-P9lX6k0K-DNi_jEUI.js';
...
function h(o) {
  ((l.innerHTML = ''),
    ...
    `),
        l.appendChild(e));
      const a = S(k, `reports/${t.id}/daily.pdf`);
      v(a)
        .then(n => {
          const r = document.getElementById(`reportLink-${t.id}`);
          ((r.href = n), (r.textContent = '📄 Daily Report (PDF)'));
        })
        .catch(() => {
...
window.requestContact = async function (o, t) {
  const e = d(c, 'contactRequests', `${s}_${o}`);
  (await setDoc(e, {
    scoutId: s,
    playerId: o,
    coachId: t,
...
window.searchReports = async function () {
  ...
  ((e.innerHTML = '<h3>🔍 Results:</h3>'),
    (await y(f(c, 'scoutingReports'))).forEach(a => {
      const n = a.data();
...
//# sourceMappingURL=scout-CXLo_sFF.js.map
