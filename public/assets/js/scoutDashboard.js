...
(async function checkApproval() {
  ...
  if (!snap.exists()) {
    document.body.innerHTML = '<h2 style="text-align:center;margin-top:100px;">❌ Scout profile not found.</h2>';
    return;
  }
...
window.searchReports = async function () {
  ...
  snap.forEach(docSnap => {
    ...
    if ((playerQuery && !data.playerId.includes(playerQuery)) ||
        (gameQuery && !data.gameId.includes(gameQuery))) return;
...
window.downloadScoutReport = function (playerId, gameId, data) {
  ...
  docPDF.text(`Areas for Improvement: ${data.areasForImprovement.join(', ')}`, 10, 60);
