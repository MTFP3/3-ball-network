...
/* empty css              */ import './firebaseConfig-DCH0t8Yd.js';
class a {
  constructor() {
  ...
  async buildSearchIndex() {
    try {
      ...
      for (const t of e)
        (await this.db.collection(t).get()).forEach(o => {
          const n = { id: o.id, ...o.data(), collection: t };
          this.indexDocument(n);
        });
  ...
  async performSearch() {
    const e = document.getElementById('searchInput').value.trim(),
      ...
      n = document.getElementById('skillFilter').value;
    let s = [];
    (e ? (s = this.textSearch(e)) : (s = await this.getAllDocuments()),
      (s = this.applyFilters(s, {
        role: t,
        location: i,
        position: o,
        skill: n,
    ...
    return (
      t.forEach(o => {
        this.searchIndex.has(o) &&
          this.searchIndex.get(o).forEach(n => {
            i.add(n);
          });
      }),
      Array.from(i)
  ...
  async getAllDocuments() {
    ...
    for (const i of t)
      (await this.db.collection(i).limit(20).get()).forEach(n => {
        e.push({ id: n.id, ...n.data(), collection: i });
      });
    return e;
  }
  applyFilters(e, t) {
