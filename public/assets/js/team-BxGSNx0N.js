...
import { b as $ } from './fanFollow-Bh7dTXqa-BOIUMshn-UU82LnyW-BOIUMshn.js';
/* empty css                                         */ const b = {
...
document.getElementById('teamTags').innerHTML = (T.tags || [])
  .map(e => `<li>${e}</li>`)
  .join('');
const l = await n(m(i(o, 'games'), r('teamName', '==', t)));
let s = 0;
l.forEach(e => {
  e.data().result === 'win' && s++;
});
document.getElementById('teamRecord').innerText =
  `Wins: ${s} / Games: ${l.size}`;
const B = await n(m(i(o, 'players'), r('team', '==', t))),
...
const D = await n(
    m(i(o, 'games'), r('teamName', '==', t), y('gameDate', 'desc'), h(3))
  ),
...
const L = await n(m(i(o, 'coaches'), r('teamNames', 'array-contains', t)));
L.forEach(e => {
  const a = e.data();
  document.getElementById('coachContact').innerText =
    `${a.firstName || 'Coach'} ${a.lastName || ''} | Email: ${a.email || 'N/A'}`;
});
//# sourceMappingURL=team-BxGSNx0N.js.map
